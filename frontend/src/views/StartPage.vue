<template>
  <div>
    ...
    <LeaseDisplayBox
      ...
      :lease="leaseItem"
    />
    <button class="populateLeasesButton" @click="populateLeases">
      Populate Leases
    </button>
  </div>
</template>
  ...
  methods: {
    ...
    checkWhichFilterToRun() {
      ...
      if (
        ...
        this.myAmountOfBeds > 0 &&
        this.myLocation.length > 0 &&
        this.myMinPrice.length > 0 &&
        this.myMaxPrice.length > 0
      ) {
    ...
    filterBasedOnLocationMinAndMaxPrice() {
      ...
      for (let lease of this.allLeases) {
        if (
          ...
          this.myMinPrice <= lease.PPPN &&
          this.myMaxPrice >= lease.PPPN
        ) {
      ...
      for (let lease of this.allLeases) {
        if (
          lease.location
            .toLowerCase()
            .includes(this.myLocation.toLowerCase()) &&
          lease.beds == this.myAmountOfBeds &&
          this.myMaxPrice >= lease.PPPN
        ) {
          this.relevantLeases.push(lease);
        }
    ...
    filterBasedOnDateMinAndMaxPrice() {
      ...
      for (let lease of this.allLeases) {
        if (
          lease.startDate <= this.startDate &&
    ...
    filterBasedOnDateBedsMaxPriceAndMinPrice() {
      ...
      for (let lease of this.allLeases) {
        if (
          lease.startDate <= this.startDate &&
          lease.endDate >= this.endDate &&
          this.myAmountOfBeds == lease.beds &&
          this.myMaxPrice >= lease.PPPN &&
          this.myMinPrice <= lease.PPPN
        ) {
          this.relevantLeases.push(lease);
    ...
    filterBasedOnDateMaxPriceAndLocation() {
      ...
      for (let lease of this.allLeases) {
        if (
          ...
          lease.endDate >= this.endDate &&
          this.myMaxPrice >= lease.PPPN &&
          lease.location.toLowerCase().includes(this.myLocation.toLowerCase())
        ) {
          this.relevantLeases.push(lease);
        }
      }
